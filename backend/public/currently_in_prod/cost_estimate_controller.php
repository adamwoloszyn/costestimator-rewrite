<?php
    ...
    class CostEstimateController {
        ...
        public function preprocessCostEstimateData($userData)
		{
		    ...
		    // Determine HighRisk
	    if($userData["HighRisk"] == "Yes"){
                $isHighRisk = true;
            } else {
                $isHighRisk = false;
            }
		    // Carrier screening: derive HighRisk from FamilyHistory if HighRisk not set
		    // Only explicit "Yes" = high risk; "No" and "I dont know" are both treated as not high risk
		    if (!$isHighRisk && isset($userData["FamilyHistory"]) && $userData["FamilyHistory"] !== "") {
		        $isHighRisk = (strtolower($userData["FamilyHistory"]) === "yes");
		    }
        ...
        public function calculateInitialCostObjectValues($fromService, $data, $estimateData){
            ...
            */
            ...
            // Carrier screening: derive HighRisk from FamilyHistory if HighRisk not set
            // Only explicit "Yes" = high risk; "No" and "I dont know" are both treated as not high risk
            if (!$isHighRisk && isset($data["FamilyHistory"]) && $data["FamilyHistory"] !== "") {
                $isHighRisk = (strtolower($data["FamilyHistory"]) === "yes");
            }
    ...
    if (!$token || $token !== $_SESSION['token']) {
        ...
        if($customerData['action'] == 'create_salesforce_record'){
            ...
            if(
                ...
                // Normalize "I don't know" (option ID 24) for question 9 (FamilyHistory) to "No" (option ID 1)
                // before sending to Phoenix. Phoenix treats option 24 differently from option 1, resulting in
                // a rule that omits Not_Covered_By_Medicial_Policy=NO. "I don't know" should behave like "No".
                if (!empty($customerData['tests']) && is_array($customerData['tests'])) {
                    foreach ($customerData['tests'] as &$_test) {
                        if (!empty($_test['questions']) && is_array($_test['questions'])) {
                            foreach ($_test['questions'] as &$_question) {
                                if ($_question['id'] == 9 && isset($_question['answer']['id']) && $_question['answer']['id'] == '24') {
                                    $_question['answer']['id'] = '1';
                                }
                            }
                            unset($_question);
                        }
            ...
            if($frontEndMessage["response"]["create_cost_estimate"]){
                ...
                // Convert MultiFetalGestation to boolean — calculateInitialCostObjectValues may have
                // already converted this to a bool; if so, keep it. If still a raw string, convert it.
                if(!is_bool($customerData["MultiFetalGestation"])) {
                    $customerData["MultiFetalGestation"] = in_array($customerData["MultiFetalGestation"], ["Twins", "Triplets", "More than three"], true);
                }

                // Convert HighRisk to boolean — same logic as above.
                if(!is_bool($customerData["HighRisk"])) {
                    $customerData["HighRisk"] = ($customerData["HighRisk"] === "Yes");
                }

                // Set NotCoveredByMedicalPolicy:
                // 1. Phoenix rule explicitly says not covered
                // 2. Inheritest tests that include the FamilyHistory question (question 9):
                //    not high risk = not covered by medical policy.
                //    Guard: FamilyHistory must be non-empty — tests like CF/SMA do not ask
                //    this question, so HighRisk defaults to false but NCBMP should NOT fire.
                //    "No" and "I don't know" both produce HighRisk=false → NCBMP=true
                //    "Yes" produces HighRisk=true → NCBMP=false
