<?php 
	    ...
	    public function createCostEstimateRecord($data, $frontEndResponse, $logger) { 
			...
			if($auth_array['http_code'] == 200){
				...
				$toBoolOrNull = function($value, $multiFetalMode = false) {
					...
					if (in_array($normalized, array('yes', 'true', '1'), true)) { return true; }
					if (in_array($normalized, array('no', 'false', '0'), true)) { return false; }
					if ($multiFetalMode && in_array($normalized, array('twins', 'triplets', 'more than three'), true)) { return true; }
					return null;
				};

				$highRiskSource = null;
				if (isset($data["HighRisk"]) && $data["HighRisk"] !== "") {
					$highRiskSource = $data["HighRisk"];
				} else if (isset($data["HighRisk_3"]) && $data["HighRisk_3"] !== "") {
					$highRiskSource = ($data["HighRisk_3"] == "2") ? "Yes" : (($data["HighRisk_3"] == "1") ? "No" : $data["HighRisk_3"]);
				} else if (isset($data["FamilyHistory_9"]) && $data["FamilyHistory_9"] !== "") {
					// Option ID 2 = "Yes" (high risk); ID 1 = "No", ID 24 = "I dont know" — both not high risk
					$highRiskSource = ($data["FamilyHistory_9"] == "2") ? "Yes" : "No";
				} else if (isset($data["FamilyHistory"]) && $data["FamilyHistory"] !== "") {
					// Text fallback: only explicit "Yes" is high risk
					$highRiskSource = (strtolower($data["FamilyHistory"]) === "yes") ? "Yes" : "No";
				}
				$highRisk = $toBoolOrNull($highRiskSource, false);
				if ($highRisk === null) { $highRisk = false; }

				$multiFetalSource = null;
				if (isset($data["MultiFetalGestation"]) && $data["MultiFetalGestation"] !== "") {
					$multiFetalSource = $data["MultiFetalGestation"];
				} else if (isset($data["MultiFetalGestation_8"])) {
					$multiFetalSource = ($data["MultiFetalGestation_8"] == "1") ? "No" : (($data["MultiFetalGestation_8"] !== "") ? "Yes" : null);
				}
				$multiFetalGestation = $toBoolOrNull($multiFetalSource, true);
				if ($multiFetalGestation === null) { $multiFetalGestation = false; }
				...
				$fields_pre_json = array(
					"ce" => 
						array(
							...
							"ConsentToContact" => (isset($data["ConsentToContact"]) ? ($data["ConsentToContact"] === true || $data["ConsentToContact"] === "Yes") : false),							   
							"ConsentToLeaveMessage" => (isset($data["ConsentToLeaveMessage"]) ? ($data["ConsentToLeaveMessage"] === true || $data["ConsentToLeaveMessage"] === "Yes") : false),							   
    ...
    if($auth_array['http_code'] == 200){
        ...
        if (isset($data["HighRisk"]) && $data["HighRisk"] !== "") {
            ...
            // Option ID 2 = "Yes" (high risk); ID 1 = "No", ID 24 = "I dont know" — both treated as not high risk
            $highRiskSource = ($data["FamilyHistory_9"] == "2") ? "Yes" : "No";
        } else if (isset($data["FamilyHistory"]) && $data["FamilyHistory"] !== "") {
            // Text fallback: only explicit "Yes" is high risk; "No" and "I dont know" are not
            $highRiskSource = (strtolower($data["FamilyHistory"]) === "yes") ? "Yes" : "No";
        }
        ...
        if (isset($data["MultiFetalGestation"])) {
            ...
            // Option ID 1 = "No" (not multifetal); any other ID (2=Twins, 3=Triplets, 4=More than three) = multifetal
            $multiFetalSource = ($data["MultiFetalGestation_8"] == "1") ? "No" : (($data["MultiFetalGestation_8"] !== "") ? "Yes" : null);
        }
