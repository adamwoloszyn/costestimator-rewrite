<?php
	// Send CORS headers FIRST before ANY other output
	...
	// Disable session cache headers that interfere with CORS
...
*/
	...
	//if (!$something) {	
		...
		*/
		...
		if($customerData['action'] == 'newEstimateRequest'){
			$stmt = $dbh->prepare("INSERT INTO used_tokens (token, usage_count) VALUES (?, 1)");
			$stmt->execute([$token]);
			
			
			// check for R2MSC
			if($customerData["insuranceProvider"] == "I don't have insurance" || $customerData["insuranceProvider"] == "I don't see my insurance"){
			...
			if($frontEndMessage["response"]["create_cost_estimate"]){
				...
				if($customerData["DisplayName"] == "I don't have insurance"){
					...
					//$customerData["CostEstimateCurrency"] = "USD";
					include '../../db.php';
					$partnerStmt = $dbh->prepare("SELECT * FROM trading_partners WHERE id = ?");
					$partnerStmt->execute([$customerData["TradingPartnerId"]["tp_id"]]);
