<?php
	...
	$array = json_decode($lookup['phoenix_lookup']['results']['categories'], true);

	
	// Function to recursively search for the test with ID 12
	function findTestById($tests, $testId) {
	    ...
	    foreach ($tests as $test) {
	        ...
	        if (isset($test['tests'])) {
	            $foundTest = findTestById($test['tests'], $testId);
	            if ($foundTest !== null) {
	                return $foundTest;
	            }
	        }
	...
	// Find the test with ID 12
	$testId = 10;
	$foundTest = findTestById($lookup['phoenix_lookup']['results']['categories'], $testId);
	
	if ($foundTest !== null) {
	    // Test with ID 12 is found
	    echo "Found Test: " . $foundTest['displayName'];
	} else {
	    // Test with ID 12 is not found
	    echo "Test not found";
	}
