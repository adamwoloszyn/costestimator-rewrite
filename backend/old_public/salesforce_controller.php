	    ...
	    public function updateCostEstimateRecord($data, $logger) { 
			...
			if($auth_array['http_code'] == 200){
				...
				if($data["ConsentToLeaveMessage"] == "Yes"){
					$data["ConsentToLeaveMessage"] = true;
				}else{
					$data["ConsentToLeaveMessage"] = false;
				}
				if($data["ConsentToContact"] == "Yes"){
					$data["ConsentToContact"] = true;
				}else{
					$data["ConsentToContact"] = false;
				}
				if($data["MultiFetalGestation"] == "Yes"){
					$data["MultiFetalGestation"] = true;
				}else{
					$data["MultiFetalGestation"] = false;
				}
				...
				//if we have the access token, let's update our cost estimate record
				$fields = json_encode(
					array(
						"ce" => 
							array(
...
*/
								...
								"CostEstimateCurrency" => (isset($data["CostEstimateCurrency"]) ? $data["CostEstimateCurrency"] : ""),
								"TestName" => (isset($data["TestName"]) ? $data["TestName"] : ""),
								"ResponseForFrontEnd" => ($data["description_of_message"] ? $data["description_of_message"] : ""),
