<?php 
    ...
    if($auth_array['http_code'] == 200){
        ...
        if (isset($data["HighRisk"]) && $data["HighRisk"] !== "") {
            ...
            // Option ID 1 = "No"; any other non-empty ID (Yes, I dont know) = high risk
            $highRiskSource = ($data["FamilyHistory_9"] == "1") ? "No" : "Yes";
        } else if (isset($data["FamilyHistory"]) && $data["FamilyHistory"] !== "") {
            // Text fallback: anything other than "No" is high risk
            $highRiskSource = (strtolower($data["FamilyHistory"]) === "no") ? "No" : "Yes";
        }
